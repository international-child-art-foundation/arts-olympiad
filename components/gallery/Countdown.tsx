...
import { getContestEndTime, getContestStartTime } from "@/utils/contest-state";
...
export const Countdown = () => {
  ...
  return (
    <div
      className={`grid grid-row mx-auto ml-12 ${countdownStyles.countdownDisplayGrid} gap-4 lg:gap-8 absolute max-w-full max-h-full lg:ml-32 top-7`}
    >
      <div className="z-50 lg:mb-0 font-semibold text-xl text-center content-center">
        {endTime
          ? "Contest Ended"
          : now < contestStartTime
            ? "Contest begins in"
            : "Contest ends in"}
      </div>
      {endTime ? (
        <div className="text-2xl font-semibold lg:font-bold">Vote Ended</div>
      ) : (
