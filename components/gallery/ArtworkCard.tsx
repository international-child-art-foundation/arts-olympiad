...
const ArtworkCard = ({ data, openModal }: ArtworkCardProps) => {
  ...
  // TODO: Build the URL from the ID
  const url = data.id;
  ...
  return (
    <div id={data.id} className="relative w-full h-full rounded-lg">
      <div className="shadow-gray-400 shadow-md rounded-lg">
        ...
        <section className="w-full h-36 xl:h-44 mxl:h-40 rounded-b-lg overflow-hidden relative">
          ...
          <div className="flex -mt-1 xl:mt-4 mxl:mt-2">
            <button onClick={() =>openModal(data.id)} className="mx-4 bg-new-blue w-full py-3 rounded text-center text-sm cursor-pointer tracking-wide text-neutral-white">
              View
            </button>
          </div>
