...
export const WisdomCarousel = () => {
  ...
  const [intersectionTarget, isTargetIntersecting, setCleanupFunctions] =
    useIntersectionObserver({ threshold: 0.2 });
  const [currentWisdom, setCurrentWisdom] = useState(0);
  const wisdomCardRefs = useRef(
    wisdomList.map(() => createRef<HTMLDivElement>())
  );

  const [wisdomText, setWisdomText] = useState(
    wisdomList[currentWisdom].wisdomText
  );
  const [authorName, setAuthorName] = useState(
    wisdomList[currentWisdom].author
  );
  ...
  useEffect(() => {
    ...
    setCleanupFunctions((prevCleanupFunctions) => [
      ...prevCleanupFunctions,
      ...
      wisdomList.forEach((wisdom, i) => {
        ...
        if (i === currentWisdom) {
          ...
          if (cardRef.current) {
            ...
            gsap.to(cardRef.current.querySelector(".cardLabel"), {
              opacity: 0,
              duration: 0.4,
            });
          }
          ...
          i === currentWisdom - 1 ||
          (currentWisdom === 0 && i === wisdomList.length - 1)
        ) {
  ...
  return (
    <figure
      className="z-5 flex flex-col justify-center items-center bg-transparent h-visionary-thinkers-md overflow-hidden "
    ...
    >
      {
        <div className=" z-5 relative w-full h-full">
          ...
          >
            <Pm className="text-sm z-10 row-span-1 text-end overflow-hidden  text-overflow-ellipsis">
              {wisdomText}
            </Pm>
            <H3m className="z-10 mt-4 text-right font-semibold overline row-span-1 xl:text-2xl lg:text-xl overflow-hidden whitespace-nowrap text-overflow-ellipsis font-montserrat pt-2">
              {authorName}
            </H3m>
          </div>
