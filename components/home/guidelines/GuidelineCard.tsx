...
export const GuidelineCard = ({
  ...
  return (
    ...
    >
      <LazyImage
        className="object-cover rounded-b-none border-b-1 border-black w-full -z-20"
        imageUrl={imgUrl}
        alt={alt || ""}
      />
      <div
        ...
        style={{
          background: `linear-gradient(to top, rgba(0, 0, 0, ${gradientStrength}), rgba(0, 0, 0, ${
            gradientStrength / 2
          }))`,
        }}
      ></div>
      <div
        ...
        className={
          "div-textholder p-6 absolute bottom-0 min-w-full max-w-[450px] lg:max-w-[unset] w-full flex flex-col gap-6 "
        }
      >
        <H3m className="font-semibold grid-card-header break-normal">
          {heading}
        </H3m>
        {description &&
          description.map((item, index) => {
            return (
              <Pm
                key={index}
                className="font-light font-sans text-sm min-w-[100%]"
              >
                {item}
              </Pm>
            );
        ...
        {button && (
          <div className="inline-flex box-border grow-0 rounded">
            <Link
              className="bg-white text-new-blue px-4 py-3 box-border rounded"
              href={button[1]}
            >
              {button[0]}
            </Link>
