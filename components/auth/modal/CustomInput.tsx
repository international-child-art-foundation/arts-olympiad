...
export const CustomInput = ({label, ...props} : CustomInputProps) => {
  ...
  return(
    <div className={`grid ${props.colSpan || props.colStart ? `${props.colSpan ? props.colSpan : ""} ${props.colStart ? props.colStart : ""}` : ""}`}>
      <label className={`mt-6 text-sm mb-1 ${meta.error && meta.touched ? "text-[#C4384E] font-semibold" : !meta.error && meta.touched && meta.value ? "text-[#158737] font-semibold" : "font-light text-neutral-black"}`}>{label}</label>
      <input 
        {...field} 
        {...props} 
