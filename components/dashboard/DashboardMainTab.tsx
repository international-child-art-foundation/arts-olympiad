...
export const DashboardMainTab: React.FC<DashboardMainTabProps> = ({
  ...
  const isContestComplete = contestState == ContestState.Complete;
  ...
  if (contestState == ContestState.Inactive) {
    return (
      <div className="w-full">
        <p className="font-montserrat text-2xl font-regular text-[32px]">
          Welcome to your dashboard!
        </p>
        <p className="text-xl my-4 mt-5">
          The contest has not yet begun. It begins on{" "}
          <span className="font-bold">
            {" "}
            {formatDate("competitionBegin", "MMMM do")}
          </span>
          .
        </p>
        <p>
          ...
          by viewing the work of our exceptional past artists!
        </p>
      </div>
    );
  } else if (
      ...
      <>
        ...
        {apiUserData &&
          ...
          dashboardLoadingState === "Loaded" && (
            <>
              <p className="font-montserrat text-2xl font-regular text-[32px]">
                Welcome to your dashboard, {apiUserData.f_name}!
              </p>
              <p className="font-light text-base py-2 pb-4">
                See your account information here.
              </p>
              ...
              {age >= 8 && age <= 20 ? (
                <>
                  {userHasPaid ? (
                    <>
                      <p className="py-2 pt-4 font-montserrat font-semibold text-2xl">
                        Your Artwork
                      </p>
                      {apiUserData.has_active_submission ? (
                        ...
                        <div>
                          <div className="my-2 w-full outline outline-1 rounded-3xl h-[400px] max-w-full flex">
                            <button
                              onClick={() =>
                                !isContestComplete && setDisplayModal("mainTab")
                              }
                              className={`z-10 flex rounded text-white p-4 m-auto items-center gap-8 ${isContestComplete ? "bg-gray-500 opacity-60 cursor-not-allowed" : "bg-new-blue"}`}
                              disabled={isContestComplete}
                            >
                              <p>Upload Artwork</p>
                              <Image
                                src={uploadIcon}
                    ...
                    <>
                      {isContestComplete ? (
                        <div className="flex flex-col my-6">
                          <span className="my-4 mx-auto text-center bg-gray-500 opacity-60 cursor-not-allowed text-white px-4 py-2 rounded">
                            Pay Entry Fee
                          </span>
                        </div>
                      ) : (
                        <div className="flex flex-col my-6">
                          <p className="">
                            In order to submit art, you must first pay a $3
                            administrative fee.{" "}
                          </p>
                          <p className="">
                            Once the fee is paid, we can go through the process
                            of getting your artwork ready for the world.{" "}
                          </p>
                          <a
                            href={
                            ...
                            className=" my-4 mx-auto text-center bg-new-blue text-white px-4 py-2 rounded active:scale-[97%]"
                          >
                            Pay Entry Fee
                          </a>
                        </div>
                ...
                <div>
                  <p>
                    You don't meet the age requirements to enter this
                    competition.
                  </p>
                  <p>
                    Feel free to vote on the{" "}
